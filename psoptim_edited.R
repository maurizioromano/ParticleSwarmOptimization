...
psoptim <- function (par, fn, gr = NULL, ..., lower=-1, upper=1,
  ...
  mrunif <- function(n,m,lower,upper) {
    return(matrix(rdunif(n*m,0,1),nrow=n,ncol=m)*(upper-lower)+lower)
  }
  norm <- function(x) sqrt(sum(x*x))
  rsphere.unif <- function(n,r) {
    temp <- rdunif(n)
    return((rdunif(1,min=0,max=r)/norm(temp))*temp)
  }
  ...
  mrsphere.unif <- function(n,r) {
    m <- length(r)
    temp <- matrix(rdunif(n*m),n,m)
    return(temp%*%diag(rdunif(m,min=0,max=r)/apply(temp,2,norm)))
  }
  npar <- length(par)
  ...
  if (p.type==0) {
    ...
    V <- matrix(rdunif(npar*p.s,min=as.vector(lower-X),max=as.vector(upper-X)),npar,p.s)
  ...
  while (stats.iter<p.maxit && stats.feval<p.maxf && error>p.abstol &&
    ...
    if (p.p!=1 && init.links) {
      links <- matrix(rdunif(p.s*p.s,0,1)<=p.p,p.s,p.s)
      diag(links) <- TRUE
    }
    ## The swarm moves
    if (!p.vectorize) {
      ...
      for (i in index) {
        ...
        if (p.type==0) {
          V[,i] <- V[,i]+rdunif(npar,0,p.c.p)*(P[,i]-X[,i]) # exploitation
          if (i!=j) V[,i] <- V[,i]+rdunif(npar,0,p.c.g)*(P[,j]-X[,i])
        } else { # SPSO 2011
          if (i!=j)
      ...
      if (any(temp)) { # improvement
        P[,temp] <- X[,temp]
        f.p[temp] <- f.x[temp]
        i.best <- which.min(f.p) #which.min(f.p)
  ...
  multiple_ret = unique(c(i.best, order(f.p)[1:3]))
  o <- list(par=P[,multiple_ret],value=f.p[multiple_ret],
